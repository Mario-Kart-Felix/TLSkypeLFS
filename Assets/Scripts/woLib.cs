...
public class woLib : IDisposable
{
    ...
    public const int MM_WOM_DONE = 0x3BD;                                           // flag used in callback to signal that the wave device has completed a buffer
    ...
    private WaveDelegate woDone = new WaveDelegate(WaveOutDone);                    // delegate for wave out done callback
                                                                                    ...
                                                                                    // difference in number between send and completed frames (queued) tells us how much data are outstanding. Clearly we need to know how much data are in each buffer plus the sample 
    ...
    unsafe static void WaveOutDone(IntPtr dev, int uMsg, int dwUser, int dwParam1, int dwParam2)
    {
        ...
        {
            (*(UInt32*)dwUser) = 0xffffffff;                                        // signal to main application that the wave device has closed
        }
    }
    ...
    public void InitWODevice(UInt32 sf, UInt32 chans, UInt32 bps, bool G711)
    {
        ...
        //.................................................................................................................................................................................
        // check is an unmanaged memory pool has been created yet. If not, create it
        ...
        {                                                                           // if not, create it. Because data must remain live while being played out, this memory must
            audioRawP = Marshal.AllocHGlobal(audioRawSize);                         // remain at a fixed location and, therefore, be unmanaged. 
        ...
        waveOutOpen(out hwo, -1, wFmt, woDone, rFrames, CALLBACK_FUNCTION);         // use this if you want the callback for frame counting, otherwise use CALLBACK_NULL
                                                                                    //        waveOutOpen(out hwo, -1, wFmt, woDone, (IntPtr) 0, CALLBACK_NULL);        // with no callback, CloseWODevive might be modified not to wait for the close signal
        ResetWODevice();
    }
    //---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
