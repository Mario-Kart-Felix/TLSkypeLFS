...
//        }
    ...
    public void execute(Map<String, String> environvenmentVars, String[] cmd, FFmpegExecuteResponseHandler ffmpegExecuteResponseHandler, int type) throws FFmpegCommandAlreadyRunningException {
        if (this.ffmpegExecuteAsyncTask != null && !this.ffmpegExecuteAsyncTask.isProcessCompleted()) {
            throw new FFmpegCommandAlreadyRunningException("FFmpeg command is already running, you are only allowed to run single command at a time");
        } else if (cmd.length != 0) {
            String[] ffmpegBinary;

            ffmpegBinary = new String[]{FileUtilsCustom.getFFmpeg(this.context, environvenmentVars, type)};
    ...
    public void execute(String[] cmd, FFmpegExecuteResponseHandler ffmpegExecuteResponseHandler, int type) throws FFmpegCommandAlreadyRunningException {
        this.execute((Map) null, cmd, ffmpegExecuteResponseHandler, type);
    }
