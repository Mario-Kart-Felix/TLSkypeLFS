#include "UniversalMediaPlayerFFmpeg.h"

@implementation UniversalMediaPlayerFFmpeg
...
{
    _player = [[MediaPlayerFFmpeg alloc] init];
...
static std::vector<UniversalMediaPlayerFFmpeg*> _players;
...
extern "C"
{
    int UMPNativeInit()
    {
        _players.push_back([[UniversalMediaPlayerFFmpeg alloc] init]);
        return (int)_players.size();
    }
    
    void UMPNativeInitPlayer(int index, char *options)
    {
        UniversalMediaPlayerFFmpeg *player = _players.at(index - 1);
	...
	bool UMPGetMute(UniversalMediaPlayerFFmpeg *mpObj)
    {
        return false;
    }
